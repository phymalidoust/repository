...
ECI_FILE = "CEIs_fcc_AlZn.json"
db_name = "alzn_fcc_3x3x3_v2.db"
...
##				comm.Barrier()
    ...
    alvc = np.arange(0.05, 0.98, 0.0143) # makes 54 steps. Then if we request 9 processors, each will do 6 steps.
                                        # if request 27 processors, each will do 2 steps
...
##            fd.close()
    ...
    for nal in range(int(strt), int(endd)):
        al0 = alvc[nal]
        zn0 = 1 - al0
        enrg = random_gs(ceBulk, al0, zn0)
        fd = open(fname, 'a+')
        print(enrg, al0, zn0, file=fd)
        fd.close()


def random_gs(ceBulk, aal, zzn):
